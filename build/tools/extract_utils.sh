...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$aosep_ROOT"/vendor/validus/build/tools/smali/baksmali.jar
        export SMALIJAR="$aosep_ROOT"/vendor/validus/build/tools/smali/smali.jar
    fi
    ...
    if [ ! -f "$aosep_TARGET" ]; then
        return;
    fi

    if grep "classes.dex" "$aosep_TARGET" >/dev/null; then
...
function extract() {
    ...
    local SRC="$2"
    local OUTPUT_ROOT="$aosep_ROOT"/"$OUTDIR"/proprietary
    ...
    if [ -f "$SRC" ] && [ "${SRC##*.}" == "zip" ]; then
        DUMPDIR="$aosep_ROOT"/system_dump
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            # If OTA is block based, extract it.
                ...
                python "$aosep_ROOT"/vendor/aosep/build/tools/sdat2img.py "$DUMPDIR"/system.transfer.list "$DUMPDIR"/system.new.dat "$DUMPDIR"/system.img 2>&1
...
function extract_firmware() {
    ...
    local COUNT=${#FILELIST[@]}
    local SRC="$2"
    local OUTPUT_DIR="$aosep_ROOT"/"$OUTDIR"/radio
